...
conn = psycopg2.connect(host="localhost", dbname="meas_write_curr",  user ="postgres", password = "numem@184", port = 5432)
        ...
        #     with open("./parsed_data/" + CHIP + "/" + TEST + "/" + save_name + ".csv", "a") as save_file:
            ...
            for line_read in decoded_data.splitlines():
                ...
                if "#DD>" in line: 
                    ...
                    dictionary["Idd18w[mA]"] = line.split(",")[18].strip()
                    dictionary["Idd18w[µA/MHz/bit]"] = float(line.split(",")[18].strip()) * 1000 / float(line.split(",")[5]) / 78
