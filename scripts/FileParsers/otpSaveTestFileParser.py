...
conn = psycopg2.connect(host="localhost", dbname="otp_save_test",  user ="postgres", password = "numem@184", port = 5432)
...
def main():
    ...
    WHERE 
        ...
        AND tf2."Test Data" = %s
    """, (lot, lot, bin, bin, wafer, wafer, process_corner, process_corner, part_num, part_num, file))
    files = cur.fetchall()
    ...
    for data_files, file in zip(raw_data_files, file_names_data):
        ...
        with open(f"./parsed_files/{CHIP}/{TEST}/{part}/{part_num}/{save_name}.txt", "wb") as save_file:
            # Retrieve bytea data
